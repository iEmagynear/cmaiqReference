...
export class EULAComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.userItem != null && !this.userItem.eula_Check){
      this.agreeView = true;
    }  else {
      console.log("False")
  ...
  eulaCheckSubmit(){
    const body = {
      'eula_Agree': this.eulaCheckAgree,
      '_id': this.userItem._id
    };
    this.api.eulaAgree(body).subscribe((dataResponse) => {
    ...
    eulaCheckValid(){
      this.eulaCheckAgree = !this.eulaCheckAgree;
    }

    setFromEula(){
      localStorage.setItem('eulaFlag', "true");
    }
