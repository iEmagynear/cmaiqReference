import { Component, OnInit, HostListener, Inject, ViewChild, ElementRef, QueryList, ViewChildren } from '@angular/core';
...
import { GoogleMapsAPIWrapper, AgmMap, MapsAPILoader, LatLngBounds, LatLngBoundsLiteral } from '@agm/core';
...
export class PresentationComponent implements OnInit {
  ...
  public lineChartColors: Color[] = [
    {
      ...
      pointBackgroundColor: '#2d4d8d',
      pointBorderColor: '#2d4d8d',
      pointHoverBackgroundColor: '#f95300',
      pointHoverBorderColor: '#f95300'
    }
  ];
  ...
  f_mls: string;
  beachesFlag: boolean;
  constructor(public mapsApiLoader: MapsAPILoader, private converterpipe: ConvertPipe, public translate: TranslateService, private api: ChartService,
        ...
        function fillBetween(a, b) {
          if (datasets[a]) {
            ...
            datasets[a].data.forEach(function (point, idx) {
              ...
              if (idx < length - 1) {
                ctx.beginPath();
                ctx.moveTo(x.getPixelForValue(datasets[a].data[idx].x), y.getPixelForValue(datasets[a].data[idx].y));
                ctx.lineTo(x.getPixelForValue(datasets[a].data[next].x), y.getPixelForValue(datasets[a].data[next].y));
                ctx.lineTo(x.getPixelForValue(datasets[b].data[next].x), y.getPixelForValue(datasets[b].data[next].y));
                ctx.lineTo(x.getPixelForValue(datasets[b].data[idx].x), y.getPixelForValue(datasets[b].data[idx].y));
                ctx.closePath();
  ...
  validateEmailForm() {
    ...
    //   subject: [homeInput + ' - cmaIQ Market Analysis ', [Validators.required, Validators.minLength(5), Validators.maxLength(100)]],
    //   email: [this.chart.client.email, [Validators.maxLength(255), Validators.required]],
    //   message: ['']
    // });
    var lower_bound = this.converterpipe.transform(this.plotResp.estimated_home_value.sales_price.lower_bound, this.currentcurrency, '1.0-0');
    var upper_bound = this.converterpipe.transform(this.plotResp.estimated_home_value.sales_price.upper_bound, this.currentcurrency, '1.0-0');
    ...
    this.emailUser = this.formBuilder.group({
      subject: [homeInput + ' - cmaIQ Market Analysis ', [Validators.required, Validators.minLength(5), Validators.maxLength(100)]],
      email: [this.chart.client.email, [Validators.maxLength(255), Validators.required]],
      message: ['I am pleased to present this home at ' + lower_bound + ' - ' + upper_bound + ". Contact me at " + email + " or " + phone + " for a showing!"]
    });
  }
  ...
  photoTooltips(tooltip) {
    ...
    if (tooltip.body) {
      ...
      var image = tooltip.title[1] ? tooltip.title[1] : 'assets/images/house-icon.png';
  ...
  initiateMap() {
    ...
    if (this.chart) {
      this.lat = (this.chart.mapDetailCenter.lat != undefined && this.chart.mapDetailCenter.lat != 0) ? this.chart.mapDetailCenter.lat : 39;
      this.lng = (this.chart.mapDetailCenter.lng != undefined && this.chart.mapDetailCenter.lng != 0) ? this.chart.mapDetailCenter.lng : -94;
      this.zoom = (this.chart.mapDetailCenter.zoom != undefined && this.chart.mapDetailCenter.zoom != 0) ? this.chart.mapDetailCenter.zoom : 5;
  ...
  initiateMapReport() {
    ...
    if (this.chart) {
      this.latReport = (this.chart.mapDetailCenter.lat != undefined && this.chart.mapDetailCenter.lat != 0) ? this.chart.mapDetailCenter.lat : 39;
      this.lngReport = (this.chart.mapDetailCenter.lng != undefined && this.chart.mapDetailCenter.lng != 0) ? this.chart.mapDetailCenter.lng : -94;
      this.zoomReport = (this.chart.mapDetailCenter.zoom != undefined && this.chart.mapDetailCenter.zoom != 0) ? this.chart.mapDetailCenter.zoom : 5;
  ...
  get_chart_details(currentId) {
    ...
    this.api.get_chart_details(body).subscribe((dataResponse) => {
      ...
      if (this.chart.relatedProperty.listing_type == 'Rental') {
        ...
        this.translate.get('Charts.Analytics.APC Probability of Rent Alt').subscribe((text: string) => {
      ...
      this.sharedMlsService.changeMlsId(dataResponse.data.relatedProperty.mls_id);
      ...
      var countOutlier = this.plotResp.active_outliers.data.length + this.plotResp.pending_outliers.data.length + this.plotResp.sold_outliers.data.length;
      if (countOutlier > 0) {

        for (var i = 0, lenn = this.plotResp.active_outliers.data.length; i < lenn; i++) {
        ...
        for (var i = 0, lenn = this.plotResp.pending_outliers.data.length; i < lenn; i++) {
          ...
          var closedHouseIdx = this.chart.pendingHomes.indexOf(removePoint[0]);
          this.chart.pendingHomes.splice(closedHouseIdx, 1);
          this.chart.relatedHomes.splice(houseIdx, 1);

        }

        for (var i = 0, lenn = this.plotResp.sold_outliers.data.length; i < lenn; i++) {
      ...
      for (var i = 0, lenn = this.plotResp.estimated_home_value.probability_of_sale_table.length; i < lenn; i++) {
        var probability_of_sale = this.plotResp.estimated_home_value.probability_of_sale_table[i].probability_of_sale;
        arr.push(probability_of_sale.toFixed(2))
        var days_on_market = this.plotResp.estimated_home_value.probability_of_sale_table[i].days_on_market;
        arr2.push(days_on_market.toFixed(0))
        this.lineChartLabels.push(this.converterpipe.transform(this.plotResp.estimated_home_value.probability_of_sale_table[i].list_price, this.currentcurrency, '1.0-0'));
      }
      ...
      this.activeCount = (this.plotResp.active_listing) ? this.plotResp.active_listing.data.length : 0;
      this.pendingCount = (this.plotResp.under_contract) ? this.plotResp.under_contract.data.length : 0;
      this.closeCount = (this.plotResp.sold_listing) ? this.plotResp.sold_listing.data.length : 0;

      var dataArray2 = [];
      this.estimated_home_value = plotResp.estimated_home_value;


      var nameArr = this.estimated_home_value.projected_date_of_sale.split(' ');

      this.translate.get('NewCharts.' + nameArr[0]).subscribe((text: string) => {
        this.estimated_home_value.projected_date_of_sale = text;
      });

      this.estimated_home_value.projected_date_of_sale = this.estimated_home_value.projected_date_of_sale + ' ' + nameArr[1];

      this.monthValue = this.estimated_home_value.months_of_inventory.toFixed(1);
      this.monthValueWhole = Math.round(this.monthValue);
      if (this.chart.relatedProperty.listing_type == 'Rental') {
        if (this.monthValue < 2.5) {
          this.monthText = "<span class='font-bold'>strong landlords’s market</span> in which demand far exceeds supply and favors the landlord in price negotiations.";
          this.monthBool = true;
          this.marketType = "Strong Landlords";
          this.translate.get('Charts.Market Type.' + this.marketType).subscribe((text: string) => {

            this.marketTypeText = text;
          });
        } else if (2.5 <= this.monthValue && this.monthValue < 4.5) {
          this.monthText = "<span class='font-bold'>landlords’s market</span> in which demand exceeds supply and slightly favors the landlord in price negotiations.";
          this.monthBool = true;
          ...
          this.monthText = "<span class='font-bold'>neutral market</span> in which supply and demand are in balance and neither party has a significant advantage in negotiations.";
          this.monthBool = true;
          ...
          this.monthText = "<span class='font-bold'>tenant’s market</span> in which supply exceeds demand and slightly favors the tenant in price negotiations.";
          ...
          this.monthText = "<span class='font-bold'>strong tenant’s market</span>  in which supply far exceeds demand and favors the tenant in price negotiations.";
          this.monthBool = false;
        ...
        if (this.monthValue < 2.5) {
          this.monthText = "<span class='font-bold'>strong seller’s market</span> in which demand far exceeds supply and favors the seller in price negotiations.";
          this.monthBool = true;
          ...
          this.monthText = "<span class='font-bold'>seller’s market</span> in which demand exceeds supply and slightly favors the seller in price negotiations.";
          this.monthBool = true;
          ...
          this.monthText = "<span class='font-bold'>buyer’s market</span> in which supply exceeds demand and slightly favors the buyer in price negotiations.";
          this.monthBool = false;
          ...
          this.monthText = "<span class='font-bold'>strong buyer’s market</span>  in which supply far exceeds demand and favors the buyer in price negotiations.";
          this.monthBool = false;
      ...
      this.lineChartOptions.scales.yAxes[0].scaleLabel.labelString = this.capPOS;
      this.lineChartOptions2.scales.yAxes[0].scaleLabel.labelString = this.capDOM;

      this.lineChartOptions.scales.xAxes[0].scaleLabel.labelString = this.suggestedLPText;
      this.lineChartOptions2.scales.xAxes[0].scaleLabel.labelString = this.suggestedLPText;
      ...
      if (this.rentalChartDisplay === true) {
        ...
        this.scatterChartOptions.scales.yAxes[0].scaleLabel.labelString = "Rental Price";
      } else if (this.landDataCheck === true) {
        //console.log("Land Check")
        this.scatterChartOptions.scales.xAxes[0].scaleLabel.labelString = "Acreage";
      } else {
        this.scatterChartOptions.scales.yAxes[0].scaleLabel.labelString = "Sales Price";
      }
        ...
        callbacks: {
          ...
          label: function (tooltipItem, data) {
            ...
            if (tooltipItem.datasetIndex === 0) {
              ...
              let newValue: string = currencyPipe.transform(price, 'USD', 'symbol', "1.0-2");
  ...
  createMarkers(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (arr[i].lat === null || arr[i].long === null) {
        ...
        var latlng = new google.maps.LatLng(parseFloat(arr[i].lat), parseFloat(arr[i].long));
  ...
  createMarkerTooltip(marker) {
    ...
    marker.addListener('mouseover', function (e) {
      ...
      console.log(infowindow.open(this.mapObj, marker));
    });
  ...
  infoWindowGenerate(listingId) {
    ...
    if (this.chart.targetProperty.category === 'rental') {
      contentString =
        '<div id="content">' +
        '<h5 class="mapDetail-text-head">' + this.currentHouse2.address + '</h5>' +
        '<div>' +
        '<img src="' + this.currentHouse2.photos[0] + '" class="mapDetail-image" >' +
        '<div class="mapDetail-text-body"> <p>' + this.squarefootage + ': ' + currentHouseSqFt + '</p></div>' +
        '<div class="mapDetail-text-body"><p> Rental Price: ' + this.converterpipe.transform(currentHousePrice, this.currentcurrency, '1.0-0') + '</p></div>' +
        '</div>' +
        '</div>';
    } else {
      contentString =
        ...
        '<img src="' + this.currentHouse2.photos[0] + '" class="mapDetail-image" >' +
        '<div class="mapDetail-text-body"><p> ' + this.squarefootage + ': ' + currentHouseSqFt + '</p></div>' +
        '<div class="mapDetail-text-body"><p> ' + this.salesprice + ': ' + this.converterpipe.transform(currentHousePrice, this.currentcurrency, '1.0-0') + '</p></div>' +
        '</div>' +
        '</div>';
    }
  ...
  showMessageFromChild(message: any) {
    ...
    this.translate.get('NewCharts.Search Error (Data)').subscribe((text: string) => {
      this.searcherrordata = text;
    });

    this.translate.get('NewCharts.Search Error (Nothing found)').subscribe((text: string) => {
    ...
    this.translate.get('NewCharts.errortextnothing').subscribe((text: string) => {
      this.searcherrornothinghtml = text;
    });

    this.translate.get('Sign Out.Confirm Delete Property').subscribe((text: string) => {
      //;
      this.ConfirmDeleteProperty = text;
      //;
    });

    this.translate.get('Sign Out.Confirm Delete Propertytext1').subscribe((text: string) => {
      //;
      this.ConfirmDeletePropertytext1 = text;
      //;
    });

    this.translate.get('Sign Out.Confirm Delete Propertytext2').subscribe((text: string) => {
      //;
    ...
    this.translate.get('Charts.Analytics.APC Suggested List Price').subscribe((text: string) => {
    ...
    this.translate.get('Charts.Analytics.APC Days on Market').subscribe((text: string) => {
    ...
    this.translate.get('Charts.Market Type.' + this.marketType).subscribe((text: string) => {
      ...
      if (this.rentalChartDisplay === true) {
        ...
        this.translate.get('Charts.Analytics.APC Probability of Sale Alt').subscribe((text: string) => {
          this.capPOS = text.toUpperCase();
          this.get_chart_details(this.currentId);
        });
      }
  ...
  deleteFromChart(mlsNumber, status) {
    ...
    Swal.fire({
      ...
      text: this.ConfirmDeletePropertytext1 + house[0].address + this.ConfirmDeletePropertytext2,
  ...
  //     this.router.navigate(['/new-chart/property-value-analysis/' + this.currentId + '/redirectBackToAnalysis'])
  //   }
  //   else{
  //     this.router.navigate(['/new-chart/property-value-analysis/' + this.currentId]);
  //   }
  //   //
  // }
  ...
  backtochart() {
    ...
    //     this.router.navigate(['/analysis/'+remember_investment_id+'/redirectBackToAnalysis/']);
    //   }
    //   else{
  ...
  showAgent() {
    this.showHideAgent = !this.showHideAgent;
  }
