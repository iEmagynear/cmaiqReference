...
export class PropertiesUploadComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<PropertiesUploadComponent>,
  ...
  /* myUploader(event, fileUpload): void {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.api.properties_import(body).subscribe((dataRes) => {
  ...
  myUploader(event, fileUpload): void {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.loader = true;
        if (fileToUpload.name.indexOf('.csv') !== -1) {
          this.api.properties_import(body).subscribe((dataRes) => {
            //console.log(dataRes);
            if (dataRes.id) {
              console.log(dataRes.id);
              this.dialogRef.close(dataRes.id);
            }
          },
            (error) => {
              this.loader = false;
              ...
              //   Swal.fire({
              //     title: "Invalid File",
              //     html: '<h6>File not contain any zip file or file is invalid. <h6>',
              //     width: '42em',
              //     customClass: 'datamismatch'
              //   });
              // }
          ...
          this.api.properties_import_zip(body).subscribe((dataRes) => {
            //console.log(dataRes);
            if (dataRes.id) {
              //console.log(dataRes.id);
              this.dialogRef.close(dataRes.id);
