...
export class TaxBenefitComponent implements OnInit {
  ...
  constructor(private converterpipe: ConvertPipe,private sharedMlsService: SharedMlsService,private _decimalPipe: DecimalPipe,
    ...
    ngOnInit() {
      ...
      this.translate.get('APC Investment Analysis.Tax Benefit / Liability').subscribe((text2: string) => {
        //console.log(text2);
        this.CashFlow = text2;
    ...
    calculateAveragePropertyValue(){
      if(this.chartdata.investment.propertyValueRes_chart_id){
        ...
        //this.api.get_chart_details(body).subscribe((dataResponse) => {
          ...
          //this.AnnualDepreciation = Math.ceil(this.BuildingValue/this.UsefulLife);
    ...
    if(this.chartdata.investment.LoantoValueSm == '$'){
      ...
      for (let index = 1; index <= 30 * 12; index++) {
        var rate = dic26 / 12;
        var period = index;
        var num_of_per = dic27*12;
        var present_value = dic25;
      ...
      for (let index = 0; index < this.Fmterm; index++) {
        ...
        this.TaxSavings.push( -Math.round( (this.GrossOperatingIncome[index]-this.TotalExpenses[index]-this.AnnualDepreciation[index])*this.NominalTaxRate) );
        ...
        { */
          (this.barChartDataPrincipalPaydown[0].data as number[]).push( -(this.convertTONumber(this.GrossOperatingIncome[index],'1.0-0')-this.convertTONumber(this.TotalExpenses[index],'1.0-0')-this.AnnualDepreciation[index])*this.NominalTaxRate );        
        /* } */
